...
class Userlog(db.Model):
    ...
    ip = db.Column(db.String(100))  # 登入IP号
...
class Movie(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    comments = db.relationship('Comment', backref='moive')  # 评论外键关系关联
...
class Admin(db.Model):
    ...
    oplogs = db.relationship('Oplog', backref='admin')  # 操作员日志外键关系关联
...
class Adminlog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    admin_id = db.Column(db.Integer, db.ForeignKey('admin.id'))  # 所属角色
    ip = db.Column(db.String(100))  # 登入IP号
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
